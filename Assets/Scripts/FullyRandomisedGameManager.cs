...
public class FullyRandomisedGameManager : GameManager
{
    ...
    IEnumerator StartUp()
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            AddPiece(randomisedWhitePieces[Random.Range(0, randomisedWhitePieces.Count)], white, i, 1);
            yield return new WaitForSecondsRealtime(.1f);
        }
        ...
        for (int i = 0; i < 8; i++)
        {
            AddPiece(randomisedBlackPieces[Random.Range(0, randomisedBlackPieces.Count)], black, i, 6);
            yield return new WaitForSecondsRealtime(.1f);
        }
